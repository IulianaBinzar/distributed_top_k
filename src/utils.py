...
def df_to_tensors(data_frame: pd.DataFrame, seq_len: int, node_count: int, k: int):
    ...
    padded_data = {col: data_frame[col].map(lambda x: pad_to_k_and_mask(x, k)) for col in data_frame.columns}
    ...
    flattened_df = pd.DataFrame({col: padded_df[col].map(lambda x: torch.tensor(x, dtype=torch.float32)) for col in padded_df})
    ...
    flattened_mask = pd.DataFrame({col: mask_df[col].map(lambda x: torch.tensor(x, dtype=torch.float32)) for col in mask_df})
